...
class CrashMigrationApp(RawAndProcessedCopierApp):
    ...
    required_config.source_dumps = Namespace()                                                                    
    required_config.source_dumps.add_option('crashstorage_class', doc='', default=None, from_string_converter=class_converter)
    ...
    def _setup_source_and_destination(self):
        super(CrashMigrationApp, self)._setup_source_and_destination()

        try:
            self.source_dumps = self.config.source_dumps.crashstorage_class(
                self.config.source_dumps,
    ...
    def _transform(self, crash_id):
        try:
            ...
            if self.config.no_dumps:
                ...
                dumps = self.source_dumps.get_raw_dumps(crash_id)
